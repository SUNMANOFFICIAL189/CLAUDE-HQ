...
- **Reaper (reap_stuck_jobs) is NOT ledger-aware** (Fable lens-2 F3 / lens-3 F4, LOW-holds-today). Unlike requeue_bid_job, the stale-heartbeat reaper requeues with no ledger check, so a worker that CRASHED mid-spend (can't self-abort) could be reaped + re-claimed + re-spend. Safe today only by the timing invariant (self-abort at 90s heartbeat staleness << reaper 900s, so a live worker stops spending long before reap). Defense-in-depth: give reap_stuck_jobs the same `status <> 'blocked'` guard (a crashed-mid-spend job terminal-fails instead of re-spending). Changes crash-recovery semantics — do deliberately.
