...
## [Open] — 2026-09-04 — insta-notion-sync: proof-check LOW residuals + loose ends (permanent-auth wave)

**What:** From `~/projects/insta-notion-sync/docs/reviews/adversarial-review-2026-09-04.md` (gate FAIL; C/H/M go to fix ticket T6, these do not): (L3) `src/cookie_refresh.py:85-90` space-delimited Netscape fallback truncates cookie values containing spaces (affects only the changed/unchanged comparison on non-standard jars); (L4) a bad cycle can run several minutes (up to 3 canary probes × 60 s + a 90 s refresh) beyond the 120 s LaunchAgent interval — launchd never overlaps, but the "every 2 minutes" promise degrades during outages; loose ends never exercised by tests or live: a real `yt-dlp --cookies-from-browser` run under launchd, a real Telegram send from the processor, whether yt-dlp still writes the `--cookies` jar when the probe URL 404s, `ai_layer.enrich_video` has no overall timeout, Notion `rich_text contains` case-sensitivity for the "login required" recovery filter. Side observation: `pgrep -f watchdog/listener.py` found NO running Telegram listener at 2026-09-04 ~18:00 (alert SENDING does not depend on it; Telegram COMMANDS do) — verify the listener LaunchAgent separately.
**Why:** None blocks landing; each is a sharp edge that becomes real only in unusual conditions.
**Estimate:** ~1 h total (sonnet) after T6 lands.
**How to start:** read the review §LOW + §Loose ends; add a `test_cookie_refresh` case for a value with spaces; cap the canary list at 2 + probe URL; add an `enrich_video` timeout; probe the Notion filter with a mixed-case fixture row in a scratch DB.
**Acceptance:** each item has either a test or a documented "won't fix" line in the review file; listener state confirmed.
**Addendum 2026-09-05 09:10 +08 (listener side-observation RESOLVED — VERIFIED-STALE):** `pgrep -fl watchdog/listener.py` → PID 15515 running; `launchctl list` shows `com.claude-hq.watchdog.listener` loaded (exit 0). The 09-04 ~18:00 "NO running listener" observation is not reproducible today; treat it as a transient or a mis-probe, not an open fault.

## [Open] — 2026-09-05 — visual-style-lock twin-copy drift check (automate the notify-on-change rule)

**What:** `visual-style-lock` now exists as two deliberate byte-identical copies (`/Volumes/Elements/My_Stuff/OFFLIMITS/.claude/skills/visual-style-lock` and `~/claude-hq/skills/visual-style-lock`, the latter symlinked into `~/.claude/skills/`). The operator's rule is: edit one → mirror the other → notify. Today that is doctrine-enforced only (memory note `feedback_visual_style_lock_twin_copies.md` + a TWIN COPIES header in the skill). Add a mechanical check: a SessionStart (or `/sync`) step that runs `diff -rq` on the pair when the drive is mounted and prints a one-line plain-English warning if they differ (or if the drive is absent, says so).
**Why:** a doctrine rule that depends on the editing session remembering it is Lesson-20 vibes; a 1-line diff makes drift visible at the next activation regardless.
**Estimate:** ~20 min (haiku/sonnet) — a shell snippet in the existing session-start hook + one test with a deliberately altered byte.
**How to start:** read `~/claude-hq/watchdog/hooks/session-end.sh` and the SessionStart hook in `~/.claude/settings.json` for the hook pattern; add `scripts/twin-check.sh` taking the two paths; wire it; test by appending a space to one copy and confirming the warning, then revert.
**Acceptance:** with identical copies → silent; with a 1-byte difference → one plain-English line naming both paths; with the drive unmounted → one line saying the OFFLIMITS twin could not be checked. No secrets, no jargon (Lesson 16).
