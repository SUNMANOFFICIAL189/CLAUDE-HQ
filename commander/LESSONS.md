...
### 39. A swapped paragraph deletes every invariant that was hiding inside it
- **Rule:** the visual-style-lock method ("invariants verbatim, one variable clause") only holds if you first ENUMERATE what the variable paragraph was carrying. Before substituting any block of a master prompt, diff the old block against the new one and re-home every constant it mentioned. Then prove it: grep the ACTUAL SENT payload for each invariant's distinctive word and require a hit per generation, not per template.
- **Why:** 2026-09-05, FLIGHTCLUB.33 pool film. The master scene prompt named the set dressing (a folded towel on the pool coping, one floating leaf) inside its COMPOSITION paragraph. The per-shot rule replaced COMPOSITION wholesale, so all four stills were generated with those words absent — the renders came back as an edgeless field of water and the operator caught it ("I expect it to mirror everything exactly"). The template still contained the words; only the payload did not. A second clause in an untouched paragraph ("he is the only thing in the pool") independently pushed the same way.
- **How to apply:** (1) sections of a master prompt are typed — SUBJECT/IDENTITY/WARDROBE/SET/LIGHT/GRADE are invariant, only FRAMING is variable; put set dressing in its own invariant section, never inside the framing block; (2) after building the payload, grep it for each invariant's distinctive noun and assert a count equal to the number of generations (the template is not the evidence — Lesson 34); (3) when an element must persist, say it persists ("part of the set, must appear in every shot"), because a reference image alone does not hold composition on a regenerating edit model; (4) check that no other paragraph contradicts it. Pairs with Lesson 29 (sweep to empty), 34 (measure the output, not the instruction).
- **Amendment (same day):** the agent DID see the missing element in the first render and reported it as "a small difference" before moving on; the operator had to call it a defect. When the render is visible to the agent, run the inventory as a checklist and treat any absent fixed element as FAIL — a difference you can name against the anchor is not a footnote, it is the result of the test.
- **Amendment 2 (2026-09-05, same day, the over-correction):** having lost the set dressing to a paragraph swap, I hoisted it into a "must appear in EVERY shot" invariant — and the next render forced a towel onto the deck behind a side-profile shot whose camera angle would never see it. The operator: "not every scene requires the towel and the leaf — it depends on where the camera is facing." An invariant of the WORLD (the set is consistent) is not an invariant of the FRAME (the set is in view). Classify elements ALWAYS / SET (framing-conditional) / VARIABLE, keep a per-shot visibility map, and check each render against ITS list — never a single global list. The fix for "it got deleted" is "it survives the edit", not "it is forced into every frame".

### 40. A framing clause is complete or it is wrong — state axis, height, side, tilt and distance
- **Rule:** when a prompt (or a ticket, or a shot list) specifies a camera setup, it must state: the camera AXIS relative to the subject (overhead / frontal / lateral-perpendicular), HEIGHT (overhead / head height / eye level / water level), the subject's SIDE shown (left/right) where relevant, TILT (level / down / up), and DISTANCE (shot size); for planimetric styles add "squared to the frame, level horizon". A bare shot name is not a spec.
- **Why:** 2026-09-05, FLIGHTCLUB.33 pool film. The Shot 3 clause said "side profile … camera at the height of the float, level with the man and off to one side". The render came back as the RIGHT side of his face, in three-quarter view, from slightly above — three unstated dimensions, three defaults the model chose. The operator's art direction was a complete left-side lateral profile with the camera parallel to his body and level. Seven cents wasted is trivial; the pattern (the engine fills every unstated camera dimension with its own default, usually a three-quarter high angle) is not.
- **How to apply:** (1) every FRAMING clause carries all five dimensions explicitly, in the project's style vocabulary; (2) the post-render check verifies the framing was honoured (axis, side, tilt) as a FAIL condition, not only element presence; (3) when the operator supplies an art-direction document, quote its camera language verbatim into the clause rather than paraphrasing it. Pairs with Lesson 34 (measure the result) and 39 (typed sections; the set is consistent, its visibility is not).
- **Amendment 2026-09-05 (the operator's correction):** "state every dimension" is not "pin every dimension". An identical prompt with the shot size unstated produced a tight frame on one run and a wide frame on the next — both faithful readings. The operator's ruling: shot size is dictated per shot by the storyboard, so a blanket pin is wrong. The honest rule is therefore about EXPECTATIONS, not completeness: a stated dimension is repeatable; an unstated one is delegated to the anchor and WILL vary. Name it when the shot changes it or must reproduce it; leave it when the anchor already holds what you want. The failure is never "unstated", it is "unstated and assumed fixed". Corollary: reusing a recipe verbatim only guarantees repeatability for the dimensions the recipe actually states. I briefly built a hard framing-completeness gate that refused any camera move without all five dimensions — the operator rejected it as turning a creative call into a lock, and it was removed.
