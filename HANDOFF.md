...
- Last refreshed: 2026-09-03 20:40 +08 — landed + hardening patch published — manual
...
1. ⭐ NEXT (operator-confirmed 20:10; landing DONE 20:20 — main @ 43919af, `hermes-pilot` published @ 12f63a2, vault 0/0): S2 = **T6** (sonnet, read-only mechanisms/schema ticket on `repos/hermes-agent` @ `hq-hardened` fc696db: items a–l in PLAN v2.1j) preceded by a tiny **T3g** (enforce `HERMES_ENGINE_DOCKERFILE_PATH_OVERRIDE` as test-only — BACKLOG R1). Via `hq-foreman`, one wave per session; re-estimate token bands ×3–4.
...
- Git: main tree `~/claude-hq` on `main` @ `43919af` = origin/main (clean, untracked scratch only); pilot worktree `run/pilot-tree` on `hermes-pilot` @ `12f63a2` = origin/hermes-pilot (published 20:20, clean); Hermes checkout `repos/hermes-agent` on `hq-hardened` @ `fc696db` (local gitignored checkout — reproducible from `run/pilot-tree/skills/hq-hermes/patches/hq-hardened.patch`, apply-checked), clean except the known case-collision file `contributors/emails/agent@Agents-Mac-mini.local`. [firsthand][confirmed]
