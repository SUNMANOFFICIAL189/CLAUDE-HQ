...
| **Client-facing copy / doctrine edit** *(added 2026-07-22, PG deck-v2 Gate-C review)* | words that ship to a client, or edits to the doctrine docs that govern them (deck copy, pitch text, published captions; voice guides, briefs, flow docs) | (1) **every factual claim traces to a source doc that actually says it** — compression is fine, invention is the #1 defect (verify quotes/counts/dates/named events against the source verbatim: a paraphrase that upgrades "a visitor complained" to "the town turned on it" is invention); (2) **zero internal-vocabulary or provenance leaks** — grep the banned lists AND read for paraphrased leaks greps can't catch; (3) **honesty labels survive** (a model stays labelled a model; counts sum; nothing implies measurement that doesn't exist); (4) **register compliance** per the governing voice doc, incl. any voice-as-subject-matter rules; (5) **no internal seam** — the copy, its brief, its flow doc, and the source docs all assert the same facts (an edit to one must sweep the others, Lesson 29); (6) **cold-reader test** — no claim that depends on citation for credibility when citation is banned, and the recipient can fact-check anything about themselves in seconds (their own words, their own numbers) so quote-them-back lines must be exact. |
